...
**Projeto:** GeoExplorer  
**Versão:** 1.7 · 12 de junho de 2026
...
| R09 | O uso real pode mostrar mais leituras à base de dados do que eu previa no início | Média | Médio | Registar chamadas importantes à base de dados, manter em cache dados estáveis, como o catálogo de locais, e reavaliar Turso/libSQL apenas com dados reais. |
...
| R14 | Exposição acidental de tokens, origens externas ou dados locais durante a entrega | Baixa | Alto | Manter `.env` fora do Git, deixar `MAPILLARY_ACCESS_TOKEN` apenas no ambiente local, configurar CORS com origens explícitas e rever ficheiros locais antes dos commits finais. |
| R15 | Carregar o catálogo completo de locais no frontend pode tornar a aplicação pesada e atrasar o arranque | Baixa | Médio | Passei o fluxo real para API e PostgreSQL, onde o backend escolhe apenas os locais necessários por sessão; o modo `mock` ficou limitado a uma amostra pequena para desenvolvimento. |
